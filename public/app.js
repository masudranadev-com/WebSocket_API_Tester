...
function selectApiRouteById(id) {
  state.selectedApiId = Number(id);
  const selectedApi = state.apiRoutes.find((item) => item.id === state.selectedApiId);
  state.apiDraft = selectedApi ? { ...selectedApi } : defaultApiDraft();
}

function selectWsEventById(id) {
  state.selectedWsId = Number(id);
  const selectedWs = state.wsEvents.find((item) => item.id === state.selectedWsId);
  state.wsDraft = selectedWs ? { ...selectedWs } : defaultWsDraft();
}
...
function renderList() {
  ...
  elements.listContainer.innerHTML = filtered
    .map((item) => {
      ...
      return `
        <article class="list-card ${item.id === activeId ? "active" : ""} rounded-[1.5rem] p-4">
          <button type="button" data-ws-id="${item.id}" class="w-full text-left">
            <div class="flex items-start justify-between gap-3">
          ...
          <div class="mt-4 flex items-center justify-between gap-3 border-t border-ink/10 pt-4">
            <span class="text-xs text-ink/45">Quick trigger</span>
            <button
              type="button"
              data-trigger-ws-id="${item.id}"
              class="${item.is_active ? "inline-flex items-center justify-center rounded-2xl bg-accent px-3 py-2 text-xs font-semibold text-white transition hover:bg-accent/90" : "inline-flex items-center justify-center rounded-2xl border border-ink/10 bg-white px-3 py-2 text-xs font-semibold text-ink/50 opacity-50"}"
              ${item.is_active ? "" : "disabled"}
            >
              ${item.is_active ? "Trigger" : "Paused"}
            </button>
          </div>
        </article>
      `;
...
async function triggerWsEvent(wsEventId = currentWs().id) {
  const targetId = Number(wsEventId);
  if (!targetId) {
    return;
  }

  if (state.selectedWsId !== targetId) {
    selectWsEventById(targetId);
    render();
  }

  const draft = state.wsEvents.find((item) => item.id === targetId) || currentWs();
  if (!draft?.id) {
    return;
  }
...
function bindEvents() {
  ...
  elements.listContainer.addEventListener("click", (event) => {
    const triggerWsButton = event.target.closest("[data-trigger-ws-id]");
    if (triggerWsButton) {
      triggerWsEvent(triggerWsButton.dataset.triggerWsId).catch((error) => setFeedback(error.message, "error"));
      return;
    }

    const apiButton = event.target.closest("[data-api-id]");
    if (apiButton) {
      selectApiRouteById(apiButton.dataset.apiId);
      render();
      return;
    }
    ...
    if (wsButton) {
      selectWsEventById(wsButton.dataset.wsId);
      render();
    }
  });
